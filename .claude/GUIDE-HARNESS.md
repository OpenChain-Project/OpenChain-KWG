...
.claude/
├── agents/
│   ├── guide-auditor.md        # 가이드 완성도·일관성 감사 (형식·구조·교차링크)
│   ├── guide-critic.md         # 콘텐츠 비판적 재검토 (Opus 4.7, 본문 정독)
...
### 2-1. 콘텐츠 비판적 재검토 (Opus 4.7, 본문 정독)
```
/guide-improve critic content/ko/guide/iso5230_guide/1-program-foundation/1-policy/_index.md
/guide-improve critic 5230       # ISO 5230 가이드 일괄
/guide-improve critic 18974      # ISO 18974 가이드 일괄
/guide-improve critic 42001      # ISO 42001 가이드 일괄 (critic 전용 지원)
/guide-improve critic enterprise # 기업 오픈소스 관리 가이드 일괄
```
`guide-auditor`가 형식·구조·교차링크를 본다면, `critic`은 본문 자체의 **모호성·누락 예외·샘플 현실성·심사 함정·실무 적용성·표준 정합성·최신성** 7개 차원을 본다. 결과는 `content/ko/guide/CRITIC-REPORT.md`에 누적 기록된다.

P1 약점이 발견되면 `/guide-improve section`으로 보강 diff 작성을 이어가는 흐름이 표준 패턴이다.
...
- **감사 / 점검 / 링크 체크 / 비판적 재검토** → 파일을 절대 수정하지 않음 (읽기 전용)
- **섹션 작성·개선** → diff 초안 제시 후 반드시 사용자 승인 대기
- **자동 기록 허용 파일** (유일하게 에이전트가 직접 쓰는 두 파일):
  - `EVIDENCE-CHECK.md` ← evidence-verifier
  - `CRITIC-REPORT.md` ← guide-critic
...
- **ISO 42001 (오픈소스 관점)**: 입증자료 번호 체계 없음(shall 요구사항 방식). 현재 하네스에서는 `guide-critic`만 42001 target을 지원하며, `audit`/`section`/`evidence`는 5230·18974·enterprise만 처리한다.
