...
<div class="oc-section__num">Part 4</div>

# 라이브 데모

<div class="oc-section__sub">도구로 직접 확인하는 오픈소스 거버넌스 — cdxgen → Dependency-Track → 취약점·라이선스 점검 · 20분</div>
...
<div class="oc-consult__kicker">Part 4 · 라이브 데모</div>
...
<div class="oc-card">
<div class="oc-card__label">왜 도구인가</div>

수작업으로는 전사 오픈소스를 추적할 수 없다. 핵심은 둘:

1. **무엇이 들어있는지** — SBOM 생성
2. **위험 지속 모니터링** — CVE·라이선스 위반 탐지

무료 도구만으로 **하루 안에** 자동화 가능.

</div>

<div class="oc-card">
<div class="oc-card__label">cdxgen + Dependency-Track</div>
<div class="oc-card__text">처음 시작 기업이 가장 빠르게 기본 자동화를 갖추는 조합. 둘 다 Apache-2.0 무료. <strong>cdxgen</strong> SBOM 생성(OWASP·CycloneDX) · <strong>Dependency-Track</strong> 중앙 분석(취약점·라이선스). CycloneDX는 5230·NTIA 요건 충족.</div>
</div>

</div>

<div class="oc-consult__source">cdxgen(OWASP) · Dependency-Track(OWASP) · Apache-2.0</div>
...
<div class="oc-consult__kicker">Part 4 · 데모</div>

# 데모 시나리오 — 생성·업로드·점검

<div class="oc-lead">SBOM을 만들어 중앙 서버에 올리고, 취약점·라이선스 위반을 한 화면에서 확인</div>
...
<div class="grid grid-cols-3 gap-4 text-sm">
  <div><strong>생성</strong> — cdxgen으로 의존성 한 번에 스캔</div>
...
<div class="oc-consult__source">둥근 사각형 = 시작/종료 · 마름모 = 분기</div>
...
<div class="oc-consult__kicker">Part 4 · 실습 ①</div>
...
<div class="oc-card">
<div class="oc-card__label">scan-upload.sh</div>
...
<div class="oc-card">
<div class="oc-card__label">핵심 포인트</div>

- **`--spec-version 1.6`** — cdxgen은 1.7 기본 생성, DT v4.14는 1.6까지라 명시 필요
- **`autoCreate=true`** — 프로젝트 자동 생성
- **API Key** — `BOM_UPLOAD` + `PROJECT_CREATION_UPLOAD` 권한

Docker만으로는 SK텔레콤 SBOM Scanner 대체 가능(CycloneDX JSON 출력).

</div>

</div>

<div class="oc-consult__source">실행: export DT_API_KEY=… 후 ./scan-upload.sh my-app 1.0.0</div>
...
<div class="oc-card">
<div class="oc-card__label">Dashboard에서 확인</div>

| 항목 | 내용 |
|------|------|
| Portfolio Vulnerabilities | 전사 취약점 심각도별 |
| Projects at Risk | 위험도 높은 프로젝트 |
| Policy Violations | 라이선스 정책 위반 |

취약점 DB(NVD·GHSA) 초기 동기화 최소 24h

</div>

<div class="oc-card">
<div class="oc-card__label">Triage — 다 긴급은 아니다</div>

<VexStatus legend />

- 사용 중 버전 아니면 → **Not Affected**
- 취약 기능 미호출 → **Not Affected**(사유 기록)
- 패치 존재 → **Exploitable**, 업그레이드 요청

</div>

<div class="oc-card oc-card--wide">
<div class="oc-card__label">라이선스 정책 위반 대응</div>
<div class="oc-card__text">WARN은 개발팀과 협의 후 Approved, FAIL은 허용 라이선스 버전으로 교체 요청. AI SBOM도 동일한 흐름으로 점검.</div>
</div>

</div>

<div class="oc-consult__source">Dependency-Track Dashboard · VEX Triage</div>
...
<div class="oc-section__num">Part 5</div>

# 마무리

<div class="oc-section__sub">핵심 회고와 첫 액션 · 15분 (Q&A 포함)</div>
...
<div class="oc-consult__kicker">Part 5 · 회고</div>

# 6대 핵심 요소, 다시 한눈에

<div class="oc-lead">오늘 다룬 거버넌스 체계는 하나의 유기적 구조</div>

<div class="oc-consult__body oc-bento oc-bento--2">

<div class="oc-card">
<div class="oc-card__label">6대 요소</div>
...
<div class="oc-card">
<div class="oc-card__label">유기적 구조</div>
...
어느 하나만으로는 작동하지 않는다 — 여섯이 맞물려야 5230·18974를 충족하는 살아있는 체계.

</div>

</div>

<div class="oc-consult__source">6대 핵심 요소 종합 · ISO/IEC 5230 · 18974</div>
...
<div class="oc-consult__kicker">Part 5 · Review</div>
...
<div class="oc-card">
<div class="oc-card__label">1 · 세 표준의 역할</div>
...
<div class="oc-card__text">표준별 <strong>각 25개, 합계 50개</strong>(공통 16 중복)로 센다.</div>

</div>

<div class="oc-card">
<div class="oc-card__label">2 · AI는 확장이다</div>
<div class="oc-card__text">기존 거버넌스 위에 AI를 <strong>얹는다</strong>. 프레임워크·모델·데이터셋 3대 영역, AI SBOM, AI 코딩, 모델 공급망이 추가 점검 대상. OSAID 1.0으로 "오픈소스 AI" 정의 — Open Weight와 구분.</div>
</div>

<div class="oc-card">
<div class="oc-card__label">3 · 준수는 회고형이다</div>
<div class="oc-card__text">자가 인증은 미래 보장이 아니라 <strong>"지난 18개월 충족해 왔음"</strong> 확인. 인증 후에도 18개월 주기로 유지·재확인.</div>
</div>

</div>

<div class="oc-consult__source">오늘의 핵심 3가지 · 5230 · 18974 · 42001</div>
...
<div class="oc-consult__kicker">Part 5 · 첫 액션</div>
...
<div class="oc-card">
<div class="oc-card__label">내일부터 할 수 있는 첫 액션</div>

1. **조직** — 담당자 1명 지정 (겸직 가능)
2. **정책** — 정책 템플릿 기반 초안 (CC BY 4.0)
3. **도구** — cdxgen + Dependency-Track으로 한 프로젝트 SBOM
4. **점검** — 입증자료 50개 커버리지 체크리스트

모든 도구·템플릿 무료 — 작은 프로젝트로 검증 후 전사 확장.

</div>

<div class="oc-card">
<div class="oc-card__label">함께하기 — OpenChain KWG</div>

한국 기업 오픈소스 컴플라이언스 담당자 커뮤니티.

- **정기 미팅** — 사례·도구·표준 동향
- **가이드·템플릿** — 강의 근거 자료 공개
- **서브그룹** — SBOM·AI·Telco 분과

</div>

<div class="oc-card oc-card--wide" style="text-align:center">
<span class="oc-stat" style="font-size:2rem">감사합니다</span>
<div class="oc-card__label">질문을 환영합니다 · Q&A</div>
</div>

</div>

<div class="oc-consult__source">OpenChain Korea Work Group · openchain-project.github.io/OpenChain-KWG</div>
