...
* 4.1.1.1: A documented Open Source Software Security Assurance policy <br>`문서화된 오픈소스 소프트웨어 보안 보증 정책`
...
1. **취약점 탐지**:
    ...
    - 복수 취약점 데이터베이스를 병행 조회하여 누락을 최소화합니다:
...
3. **대응 조치**:
    ...
    - 공급망 파트너·고객에게 영향 여부를 통지할 때는 **VEX**(Vulnerability Exploitability eXchange) 표준 형식(CSAF 2.0 또는 CycloneDX VEX)을 사용합니다. VEX는 다음 4가지 상태값으로 영향 여부를 표현합니다: `not_affected`(영향 없음·justification 필수) / `affected`(영향 있음) / `fixed`(패치 완료) / `under_investigation`(조사 중).
4. **대응 기록 유지**:
...
* 4.2.2.4: A documented procedure that assigns internal responsibilities for Security Assurance.<br>`보안 보증에 대한 내부 책임을 할당하는 문서화된 절차`
...
* 4.2.2.2: The identified Program roles have been properly staffed and adequate funding provided;<br>`프로그램 내 각 역할을 담당하는 인원이 적합하게 배치되고, 예산이 적절하게 지원되어야 합니다.`
...
* 4.2.2.3: Identification of expertise available to address identified Known Vulnerabilities<br>`식별된 알려진 취약점을 해결을 위해 전문 기술 자문을 이용할 수 있는 방법`
...
* 4.1.4.1: A written statement that clearly defines the scope and limits of the Program <br>`프로그램의 적용 범위와 한계를 명확하게 정의한 문서화된 진술`
* 4.1.4.2: A set of metrics the program shall achieve to improve<br>`프로그램 개선을 위해 달성해야 하는 일련의 측정 기준`
* 4.1.4.3: Documented Evidence from each review, update, or audit to demonstrate continuous improvement.<br>`지속적인 개선을 위해 검토, 업데이트 또는 검사를 수행했음을 입증하는 문서화된 증거`

{{% /alert %}}

기업은 조직과 제품의 특성을 고려하여 오픈소스 프로그램의 적용 범위와 한계를 명확히 정의하고, 이를 오픈소스 정책에 명시해야 합니다.

또한, 비즈니스 환경에 맞추어 변화함에 따라 조직 구조, 제품 및 서비스가 프로그램의 적용 범위를 결정하거나 수정해야 하는 상황이 발생할 수 있습니다. 기업은 적용 범위를 평가하기 위한 측정 기준(§1.4.1, ISO/IEC 18974 §4.1.4.2)을 수립하고, 지속적인 개선을 위한 검토·감사 결과를 문서화(§1.4.2, ISO/IEC 18974 §4.1.4.3)해야 합니다.
...
#### 1.4.1 성과 지표 (ISO/IEC 18974 §4.1.4.2)

프로그램의 지속적 개선을 위해 아래 지표를 정의하고, 최소 연 1회 측정·기록한다.
상세 지표 및 목표값은 §10.1을 참고한다.

- Critical 취약점 평균 대응 시간: 7일 이내
- High 취약점 평균 대응 시간: 30일 이내
- SBOM 완전성: 공급 소프트웨어의 100%
- 신규 참여자 교육 이수율: 온보딩 후 3개월 내 100%

#### 1.4.2 지속적 개선 증거 (ISO/IEC 18974 §4.1.4.3)

각 검토, 업데이트, 감사 결과를 문서화하여 보존한다.
문서에는 검토 일자, 발견된 미흡 사항, 개선 조치 및 완료 일정이 포함되어야 한다.
상세 절차는 §10.2 및 §10.3을 참고한다.
...
* 4.2.1.1: Publicly visible method to allow third parties to make Known Vulnerability or Newly Discovered Vulnerability enquires (e.g., via an email address or web portal that is monitored by Program Participants)<br>`제3자가 알려진 취약점 또는 새로 발견된 취약점에 대해 문의할 수 있는 공개된 방법 (예: 이메일 주소 또는 프로그램 참여자가 모니터링하는 웹 포털)`
...
> - [§3.1.1 정책](../../iso5230_guide/1-program-foundation/1-policy/) — 문서화된 오픈소스 정책
> - [§3.1.4 프로그램 범위](../../iso5230_guide/1-program-foundation/4-scope/) — 적용 범위 정의
> - [§3.2.1 외부 문의 접근성](../../iso5230_guide/2-relevant-tasks/1-access/) — 공개 문의 채널
> - [§3.2.2 효과적 리소스](../../iso5230_guide/2-relevant-tasks/2-resourced/) — 인원·예산·미준수 검토
> - [§3.5.1 기여](../../iso5230_guide/5-community/1-contributions/) — 기여 정책
> - [§4.1.4 프로그램 범위(18974)](../../iso18974_guide/1-program-foundation/4-scope/) — 성과 메트릭 정의
> - [§4.2.1 접근성(18974)](../../iso18974_guide/2-relevant-tasks/1-access/) — 취약점 신고 채널
>
> **AI 사용 시 추가 고려:**
> - [§7. AI 컴플라이언스](../7-ai-compliance/) — AI 정책 통합·AI 코딩 도구 사용 정책
> - [ISO/IEC 42001 §5.2 AI 정책](../../iso42001_guide/1-context-leadership/) — AI 정책에 OSS 원칙 포함
> - [AI SBOM 가이드](../../iso42001_guide/4-operation/2-ai-sbom/) — SPDX 3.0 AI Profile · CycloneDX 1.6 ML-BOM
