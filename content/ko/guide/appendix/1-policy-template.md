...
---
{{< alert title="Note:" >}}
이 샘플 오픈소스 정책은 다음 두가지 자료를 참고하여 작성하였다.
...
* 각 역할의 담당자는 자신이 역할을 수행함에 있어서 적절한 지원이 되지 않는다면 반드시 오픈소스 책임자에게 문제를 제기해야 한다. 적절하게 해결되지 않는다면, 오픈소스 운영위원회에 문제 해결을 요청할 수 있다.
...
* 오픈소스의 결합관계를 파악하여 자사의 코드가 오픈소스 라이선스의 영향을 받지 않도록 소프트웨어 아키텍쳐를 설계한다. 
...
기업은 소프트웨어 배포에 관여하는 모든 구성원이 오픈소스 정책과 다음 사항을 인식할 수 있도록 교육을 제공한다.
...
* 배포 대상 소프트웨어에 포함된 오픈소스 현황을 문서화하여 관리한다. 
* 이를 효과적으로 수행하기 위해 기업의 오픈소스 컴플라이언스 프로세스를 준수한다.

오픈소스 라이선스 준수를 위한 과정에서 의문사항이 있을 경우 소프트웨어 개발팀은 오픈소스 센터에 문의한다. 
* 일반적이지 않은 사용 사례여서 법적 해석이 명확하지 않을 경우, 오픈소스 센터는  법무팀에게 문의 할 수 있다.
...
기업은 오픈소스에서의 비즈니스 가치 창출을 위해 외부 오픈소스 프로젝로의 참여와 기여를 권장한다. 그러나 이 과정에서 의도하지 않은 기업의 지식 재산 노출 혹은 제3자의 권리 침해에 주의해야 한다. 이를 위해 기업의 구성원이 외부 오픈소스 프로젝트로의 기여를 위해서는 다음 사항을 준수해야 한다.
...
* 이 오픈소스 정책은 OpenChain 규격 2.1에 적합하도록 설계되었다.
* 기업은 OpenChain 규격 2.1의 모든 요구사항을 준수한다. 
* 기업은 OpenChain 적합성을 선언한 이 후 최소 18개월 이상 OpenChain 규격 2.1의 모든 요구사항을 준수하기 위해 정책 및 프로세스를 계속 개선하며 유지한다.