...
이 조항은 **Phase 3 — 프로세스 구축** 단계에서 구축합니다. ISO/IEC 18974 전용 신규 조항으로, 8가지 취약점 처리 방법 각각에 대한 문서화 절차를 수립합니다.
...
7. **위험 해결 검증**: 식별된 위험이 출시 전에 해결되었는지 검증하는 방법을
   정의한다.
8. **위험 정보 보고**: 적절한 경우 제3자에게 위험 정보를 내보내는 방법을 정의한다.

## 3. 요구사항 및 입증자료

| 조항 번호 | 요구사항 (KO) | 입증자료 |
|-----------|--------------|---------|
| §4.1.5 | 프로그램은 알려진 취약점의 건전하고 견고한 처리 절차와 보안 소프트웨어 개발을 다음 8가지를 정의하고 구현하여 시연해야 한다: 위협 식별 / 취약점 탐지 / 후속 조치 / 고객 통보 / 배포 후 신규 취약점 분석 / 지속적 보안 테스트 / 위험 해결 검증 / 위험 정보 보고 | **4.1.5.1** 위에 식별된 각 방법에 대한 문서화된 절차가 존재 |
...
- OSV.dev(Google), NVD(미국 NIST), GitHub Security Advisories(GHSA),
  KISA KNVD(Korea National Vulnerability Database, 한국인터넷진흥원) 등 복수의 취약점 데이터베이스를 병행 참조한다.
- 탐지된 취약점은 CVSS 점수에 더해 EPSS(Exploit Prediction Scoring System) 점수와
  CISA KEV(Known Exploited Vulnerabilities) 등재 여부를 함께 기록한다.
...
```
[후속 조치 절차 개요]
- CVSS 점수(v3.1 또는 v4.0; 두 버전 모두 부여된 경우 더 높은 점수)에 따라 조치 우선순위와 기한을 결정한다:
...
- 자체적으로 새로운 취약점을 발견한 경우 CVD 절차에 따라 상류 프로젝트
  또는 CERT에 보고한다.
- 공급망 파트너에게 취약점 영향 정보를 공유할 때는 VEX 형식(CSAF 2.0 또는
  CycloneDX VEX)을 사용한다. VEX는 다음 4가지 상태값으로 영향 여부를 표현한다:
  · `not_affected`     — SBOM 컴포넌트에 CVE는 존재하나 사용 맥락상 영향 없음(justification 필수)
  · `affected`         — 영향 있음(조치 진행 중)
  · `fixed`            — 패치 적용 완료
  · `under_investigation` — 영향 조사 중
  특히 `not_affected`는 고객·공급망 파트너의 불필요한 패치 작업을 차단하는
  핵심 신호이므로, justification(예: `vulnerable_code_not_in_execute_path`,
  `inline_mitigations_already_exist`)을 함께 기재한다.
...
8가지 방법 중 상당 부분을 자동화 도구로 처리할 수 있다.
아래 표는 각 방법과 권장 도구의 매핑을 보여준다.

| 방법 | 자동화 도구 | 비고 |
|------|-----------|------|
| 1. 구조적·기술적 위협 식별 | [Dependency-Track](../../../tools/7-dependency-track/), ORT | 의존성 트리 분석 |
| 2. 알려진 취약점 탐지 | [OSV-SCALIBR](../../../tools/4-osvscalibr/), [cdxgen](../../../tools/5-cdxgen/)+Dependency-Track, Grype | **핵심 자동화 대상** |
| 3. 취약점 후속 조치 | [Dependency-Track](../../../tools/7-dependency-track/) | 티켓·알림·조치 추적 |
| 4. 고객 통보 | VEX 생성 도구 (CycloneDX CLI 등) | VEX 형식으로 공유 |
| 5. 배포 후 신규 취약점 분석 | [Dependency-Track](../../../tools/7-dependency-track/) | SBOM 보관 + 신규 CVE 자동 대조 |
| 6. 지속적 보안 테스트 | CI/CD 파이프라인 + SCA 도구 | PR 머지 시 자동 게이트 |
| 7. 위험 해결 검증 | 동일 스캔 도구로 재실행 | 패치 후 재스캔 |
| 8. 위험 정보 보고 | CycloneDX VEX, CSAF | 표준 형식 사용 권장 |

### 핵심 도구 스택 (최소 구성)

처음 구축하는 기업에 권장하는 최소 도구 스택:

```
소스코드·의존성
      │
      ▼
[cdxgen 또는 Syft] ── SBOM 생성 (방법 2·5의 기반)
      │
      ▼
[Dependency-Track] ── 취약점 탐지·추적·알림 (방법 2·3·5)
      │
      ├─ Critical/High 탐지 → 담당자 알림 → 방법 3(후속 조치) 실행
      ├─ 신규 CVE 발행 → 기배포 SBOM 자동 대조 (방법 5)
      └─ 릴리스 게이트 → 미해결 위험 차단 (방법 7)
```

{{% alert title="도구 도입 순서" color="info" %}}
**1단계**: cdxgen 또는 Syft로 SBOM을 생성하는 파이프라인 구축
**2단계**: Dependency-Track에 SBOM을 연동하여 취약점 모니터링 자동화
**3단계**: CI/CD 보안 게이트 설정 (Critical/High 미해결 시 빌드 차단)
{{% /alert %}}
...
- 관련 도구: [OSV-SCALIBR](../../../tools/4-osvscalibr/), [Dependency-Track](../../../tools/7-dependency-track/), [cdxgen](../../../tools/5-cdxgen/)
